...
import java.util.*;
    ...
    class ProblemSolutions {
        ...
        public boolean canFinish(int numExams,
                                 int[][] prerequisites) {
            int[] inDegree = new int[numExams]; // Track in-degrees of all nodes
            ...
            int visitedNodes = 0; // Count nodes visited in topological order
            while (!queue.isEmpty()) {
                int current = queue.poll();
                visitedNodes++;

                for (int neighbor : adj[current]) {
                    inDegree[neighbor]--; // Reduce in-degree of neighbor
            ...
            // If all nodes are visited, it means no cycles exist
            return visitedNodes == numExams;
        }
     ...
     */
        ...
        private void dfs(int[][] adjMatrix, int node, boolean[] visited) {
            visited[node] = true;
            for (int neighbor = 0; neighbor < adjMatrix[node].length; neighbor++) {
